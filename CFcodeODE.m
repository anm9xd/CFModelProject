...
if CF == 1
    y0 = [1278.037, 64.7532, 15.04461, 12.75542, 1713.089, 100, 100, 100];
    % initial conditions: B = y0(1), P = y0(2), A = y0(3), M = y0(4), H = y0(5),
    % D = y0(6), LF = y0(7), and PH = y0(8)
elseif CF == 0
    y0 = [1.383617,6.091615, 2.393149, 1.075577, 2431.41, 100, 100, 100];
end
...
y02 = [3000; yode(end,2); yode(end,3); yode(end,4); yode(end,5); yode(end,6); yode(end,7); yode(end,8)];
[~,yode2] = ode45(@diffeqs,t2,y02);

t3 = 160:hr:299;
y03 = [5000; yode2(end,2); yode2(end,3); yode2(end,4); yode2(end,5); yode2(end,6); yode2(end,7); yode2(end,8)];
[~,yode3] = ode45(@diffeqs,t3,y03);

t4 = 300:hr:tf;
y04 = [10000; yode3(end,2); yode3(end,3); yode3(end,4); yode3(end,5); yode3(end,6); yode3(end,7); yode3(end,8)];
...
figure;
subplot(2,2,1);
plot(tt, [yode(:,7); yode2(:,7); yode3(:,7); yode4(:,7)], 'g') %LF
hold on;
plot(tt, [yode(:,8); yode2(:,8); yode3(:,8); yode4(:,8)], 'r') %PH
...
legend('B');
subplot(2,2,3);
plot(tt, [yode(:,2); yode2(:,2); yode3(:,2); yode4(:,2)],'color',[0 .5 0]) %A
hold on;
plot(tt, [yode(:,3); yode2(:,3); yode3(:,3); yode4(:,3)],'color',[1 .5 0]) %M
hold off;
xlabel('time')
legend('A','M');
subplot(2,2,4);
plot(tt, [yode(:,4); yode2(:,4); yode3(:,4); yode4(:,4)],'color',[0 0 .7]) %P
xlabel('time')
legend('P');
end
